    ...
    (render [this]
            (let [canvas-state (canvas/get-canvas-state @state/app-state (+ step-num 1))]
              (dom/div #js {:className "stepContainer"}
                    ...
                    (apply dom/svg #js {:width 100 :height 100 :viewBox (str "0 0 " (canvas/canvas-size 0) " " (canvas/canvas-size 1))  :className "stepSVG"}
                           (om/build-all canvas/render-canvas (vec (dissoc canvas-state :text)))))))))
